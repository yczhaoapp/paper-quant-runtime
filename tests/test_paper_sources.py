...
def test_catalog_sources_are_pinned_individually() -> None:
    ...
    assert len(lock["supporting_sources"]) == 1
    assert lock["supporting_sources"][0]["supports_strategy"] == (
...
def test_all_primary_pdfs_match_pins_when_cache_is_prepared() -> None:
    lock = json.loads(LOCK.read_text())
    if any(
        not _resolve_file(source["file"]).is_file()
        for source in [*lock["sources"], *lock["supporting_sources"]]
    ):
