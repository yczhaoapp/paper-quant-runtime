...
| 论文证据 | 18/18 项有固定原文、页级锚点、方法 spec、实现和 oracle；其中 3 份 CC BY PDF 可随仓库离线接入 | 其余原文按 SHA-256 显式准备到本地缓存；全部 18 项及一篇独立新论文通过论文深度验收，不把文本锚点误作语义复现证明 |
...
这里的 D1 是**本项目内部的数据证据分级**：固定可公开获取的原始行情字节、来源、哈希和时间切分；它不是题目官方等级，也不表示原论文实验数据复现。A/D/R/E 的完整范围与 R1、R2 不同验收边界见[四轴证据口径](evidence-axes.md)。
