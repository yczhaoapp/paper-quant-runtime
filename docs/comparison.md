...
`execution_profile_sha256` 从报告已绑定的执行设置计算，不接受调用方任意填写。设置需说明可用时间、撮合延迟、价格、手续费、滑点、现金和仓位限制；同一哈希仍需配合账户、订单和成交轨迹的差分验证。
...
另一实现应独立导出相同的 `Observation` Schema，不需要在本项目内导入其代码。动作比较保留方向、数量、限价和客户订单 ID；说明性 `reason` 与日志不参与路径一致性比较。若不同引擎使用不同的订单 ID 生成规则，应在对照前显式建立规范化映射，不能删除限价等会影响执行的字段。结果对照只能说明运行行为或数值差异；论文方法忠实度必须另行核对原文、公式、实验数据与独立 oracle。
